...
class DataHandler(object):
    ...
    def __init__(self,django_settings,connect_redis=True):
        ...
        self.config_last_loading_time = time.time()  # 最后一次更新配置文件的时间
        self.global_monitor_dic = {}  # 定义全局监控dict
    ...
    def looping(self):
        ...
        while not self.exit_flag:
            ...
            count += 1
            if time.time() - self.config_last_loading_time >= self.config_update_interval:  # 判断是否需要更新配置文件
                print("\033[41;1mneed update configs ...\033[0m")
                self.update_or_load_configs()  # 更新配置文件
            if self.global_monitor_dic:
                for h,config_dic in self.global_monitor_dic.items():  # 按主机遍历
                    print('handling host:\033[32;1m%s\033[0m' %h)
                    for service_id,val in config_dic['services'].items():  # 循环该主机所有要监控的服务
                        #print(service_id,val)
                        service_obj,last_monitor_time = val  # val = (service, last_timestamp)
    ...
    def load_service_data_and_calulating(self,host_obj,trigger_obj,redis_obj):
        ...
        expression_res_string = ''  # 定义一个逻辑表达式字符串
        for expression in trigger_obj.triggerexpression_set.select_related().order_by('id'):  # 按ID排序方便计算逻辑关系
            ...
            if single_expression_res:
                calc_sub_res_list.append(single_expression_res)  # 将计算出的单个表达式的结果放到总的结果列表里
                if single_expression_res['expression_obj'].logic_type: # 如果有与下一条的关系，就代表不是最后一条
                    # 拼接关系运算符，两边加空格
    ...
    def update_or_load_configs(self):
        ...
        :return:
        '''
        all_enabled_hosts = models.Host.objects.all()  # 从数据库中查出所有的主机
        for h in all_enabled_hosts:
            ...
            for group in h.host_groups.select_related():  # 组
                #print("grouptemplates:", group.templates.select_related())
                service_list = []
                trigger_list = []
                for template in group.templates.select_related():  # 组模板
                    #print("tempalte:",template.services.select_related())
                    #print("triigers:",template.triggers.select_related())
                    service_list.extend(template.services.select_related())  # 模板对应的service
                    trigger_list.extend(template.triggers.select_related())  # 模板对应的触发器
                for service in service_list:
                    # TODO 我感觉这里写成统一的格式会好一些，后续的判断就能减少很多代码量
        ...
        self.config_last_loading_time = time.time()  # 更新配置文件的load时间
        return True
    ...
    def trigger_notifier(self, host_obj, trigger_id, positive_expressions, redis_obj=None, msg=None):
        ...
        self.redis.publish(self.django_settings.TRIGGER_CHAN, pickle.dumps(msg_dic))  # 存redis
...
class ExpressionProcess(object):
    ...
    def process(self):
        data = self.load_data_from_redis() #已经按照用户的配置把数据从redis里取出来了, 比如 最近5分钟,或10分钟的数据
