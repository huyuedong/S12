...
with open(lock_file) as f1:
	for i in f1.readlines():
		lock_list.append(i.strip())  # 去掉每一行的空格和回车，生成锁定用户列表
...
while is_not_locked:  # 不是锁定用户时继续执行下面程序
	with open(account_file) as f2:
		for line in f2.readlines():
			account_list.append(line.split())  # 将account.txt文件中的每一行去掉空格后写入account_list
	for i in account_list:
		if user_name == i[0]:   # 如果是注册用户
